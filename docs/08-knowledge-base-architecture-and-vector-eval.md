...
```
                                    CLIENT SURFACES
                     ┌───────────────────────────────────────────┐
                     │ • Next.js PWA (Gym Floor Logger / Dash)   │
                     │ • Telegram Bot (Coach & Member Community) │
                     │ • Web Concierge Widget (Prospective / FAQ)│
                     └─────────────────────┬─────────────────────┘
                                           │
                                           ▼
               FASTAPI GATEWAY / CONCIERGE ENGINE (`concierge-bot-engine`)
                                           │
       ┌───────────────────────────────────┼───────────────────────────────────┐
       ▼                                   ▼                                   ▼
┌─────────────────────────┐     ┌─────────────────────────┐     ┌─────────────────────────┐
│         TIER 1          │     │         TIER 2          │     │         TIER 3          │
│   Transactional State   │     │    Literature Engine    │     │   Curated Prompt Layer  │
│   (Relational & State)  │     │   (Embedded LanceDB)    │     │  (Fast-Path & Golden)   │
├─────────────────────────┤     ├─────────────────────────┤     ├─────────────────────────┤
│ • PostgreSQL (Javier) / │     │ • 51MB Soviet Books     │     │ • Deterministic Triage  │
│   Firestore Collections │     │ • VBT Research Dossier  │     │   (90-second floor cue) │
│ • Athlete anthropometry │     │ • Meeting Transcripts   │     │ • Biometric Rules Engine│
│   (#LongFemurs, ratios) │     │ • Hybrid Search:        │     │   (torso/femur > 1.15)  │
│ • Active cycle / block  │     │   BM25 + text-embedding │     │ • Joel's Coaching Voice │
│ • Live VBT velocity logs│     │ • Latency: < 15ms       │     │ • Typography Guardrails │
│ • Latency: < 30ms       │     │ • Cost: $0.00           │     │ • Latency: < 5ms        │
└─────────────────────────┘     └─────────────────────────┘     └─────────────────────────┘
                                           │
                                           ▼
                          ...
                          │ Streaming output (TTFT < 800ms) │
                          │ Strict factual grounding filter │
                          └─────────────────────────────────┘
```

### Breakdown of the 3 Tiers

#### Tier 1: Transactional & Biomechanical State (PostgreSQL / Firestore)
* Athlete biometric data: `height_cm`, `femur_length_cm`, `torso_length_cm`, `arm_span_cm`, `ape_index`.
* Dynamic tags: `#LongFemurs`, `#ShortTorso`, `#HipImpingement`, `#PatellarTendinopathy`.
* Real-time session telemetry: `set_number`, `logged_weight`, `velocity_m_s`, $\Delta v$ decay, RPE.
* Active program state: 12-week macrocycle, current 4-week block (Accumulation / Transmutation / Realization).
* **Storage Choice:** Aligns with Javier’s PostgreSQL database (`seed-database.sql`) via JWT, with Firestore as a mobile offline-syncing alternative.
